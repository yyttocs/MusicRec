<%@ page language="java"
	import="com.sh.model.*,java.util.*,java.util.Map.Entry,com.sh.recommend.*"
...
<body>
    ...
    <!-- start: Header -->
        ...
        <div class="row-fluid">
			...
			<ul class="breadcrumb">
				<li>
					<i class="icon-home"></i>
					<a href="recsonglist1.jsp">推荐歌单</a> 
			...
			<div class="row-fluid ">		
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon user"></i><span class="break"></span>推荐歌单如下</h2>
					</div>
		...
		ItemRatingCl ir = new ItemRatingCl();
		if (ir.recommended1(s).next()) {
		
			/* System.out.println("aaaa"); */
			
				itemBean[] iBi = new itemBean[11];
				Ibcf ib = new Ibcf();
				UserBeanCl ubc = new UserBeanCl();
				int user = ubc.getUserID(s);
				...
				set.addAll(ib.recommended(user).entrySet());
				int i = 0;
				int im = 0;
				String s1 = "";
				for (Entry<Integer, Float> entry : set) {
					...
					if (i <= 10) {
						im = entry.getKey();
						s1 = ubc.getItemInfo(im);
						ItemBeanCl itb = new ItemBeanCl();
   	   					String Singer = itb.getSingerByMusicName(s1);
   	   					
   	   					String MusicName = itb.getMusicName(s1);
						itemBean iB = new itemBean();
						float rate = entry.getValue();
						int songId = entry.getKey();
						
					
						/* iB.setItemId(entry.getKey());
						iB.setItemName(s1);
						iB.setUti(entry.getValue());
						iBi[i] = iB;
						int songId = entry.getKey();
						float rate = entry.getValue();
						int rate1 = (int)rate; */
	...
	%>
			<tr>
					<td class="center"><%out.println("<a href=\"songlist1.jsp?songId="+songId+"&songName="+s1+"\">" +MusicName  + "</a><br>");%></td>
					<td class="center"><%=Singer%></td>
					<td class="center"><%=rate1%></td>
	...
	%>
							<tr>
								...
								<td class="center">你还没有评价过歌曲</td>
								<td class="center">还没有歌手信息诶</td>
								<td class="center">找点歌来吧</td>
							</tr>
						  </tbody>
					  </table>            
					</div>
